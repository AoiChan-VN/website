...
const routes = {
    home: () => `
        ...
        <section class="admin-panel-v4" style="padding: 20px 10%;">
            <div class="card ptero-card" style="background:#0d1117; border:1px solid var(--primary); overflow:hidden; position:relative;">
                <div style="padding:15px 20px; border-bottom:1px solid rgba(255,255,255,0.05); display:flex; justify-content:space-between;">
                    ...
                    <div style="color:#22c55e; font-size:0.7rem;">● LIVE</div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:repeat(3,1fr); background:rgba(0,0,0,0.3); border-top:1px solid rgba(255,255,255,0.05);">
                    ...
                    <div style="padding:20px; text-align:center;"><span>Total</span><br><b style="font-size:1.5rem; color:var(--primary);">${siteData.plugins.length + siteData.resources.length}</b></div>
                </div>
            </div>
        </section>`,
    ...
    resources: () => `<div class="page-header" style="padding: 0 10%;"><h2 class="section-title">Tài Nguyên</h2></div><div class="grid-container">${siteData.resources.map(i => `<div class="card"><div class="card-img-wrapper"><img src="${i.img}"></div><div class="card-body"><h3>${i.name}</h3><button onclick="window.viewDetail('${i.mdPath}')" class="btn-primary">Chi Tiết</button></div></div>`).join('')}</div>`,
...
const handleRender = async () => {
    console.log("Router khởi chạy...");
    const hash = window.location.hash.replace("#", "") || "home";
    const layout = document.getElementById('layout-root');
    const content = document.getElementById('page-content');

    if (!layout || !content) {
        console.error("Không tìm thấy vùng chứa HTML!");
        return;
    }
    ...
    if (hash === "home") {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (line) line.setAttribute('d', `M0,150 L100,${150 - siteData.plugins.length*20} L250,${150 - siteData.resources.length*20} L400,100`);
            }
        } catch (e) { console.warn("SVG chưa sẵn sàng."); }
    }
};

window.addEventListener("hashchange", handleRender);
window.addEventListener("DOMContentLoaded", handleRender);
// Chạy ngay lập tức để tránh lỗi DOMContentLoaded đã qua
handleRender(); 
